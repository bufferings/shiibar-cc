...
- 最上部の **⌄ メニュー**の構成(共通項目の並び順は一覧ウィンドウ表示中のアプリメニュー(下記)と
  同一。最終グループは「窓の動詞」が器で入れ替わり(⌄ = Open as Window / アプリメニュー =
  Close Window)、窓の性質である Keep on Top はアプリメニューにのみある。§8.30/§8.33):
  ...
  **Open as Window**(一覧ウィンドウを開く。下記。アプリメニューの Close Window に当たる位置。
  ⌄ 固有の項目 — 一覧ウィンドウ自体には ⌄ が無い。**窓が存在する間は disabled**)/
  **Conversations…**(会話のウィンドウを開く。§4.6。同じく**窓が存在する間は disabled**。
  セパレーターは増やさない — 最終グループは「窓の動詞 + Quit」のまま 1 行増えるだけ)/ **Quit**
...
- **一覧ウィンドウ**(⌄ → Open as Window): ドロップダウンと同じ一覧を、閉じるまで出しっぱなしに
  ...
  - **位置は毎回アイコン直下**: Open as Window を押した時点のドロップダウンパネルと同じ位置に出す
    (実装はそのパネルの画面フレームを流用してよい)。ドラッグで動かせるが**位置は記憶しない** —
    「開いたら右上(アイコンの下)に出る」が常に成り立つ。**窓が存在する間、Open as Window は
  ...
  - **アプリメニューは 1 個だけ**(File / Edit / Window メニューは出さない):
    About Shiibar CC / セパレーター / Settings…(⌘,)/ Setup Check… / セパレーター /
    Rescan(⌘R)/ Clear badges(未読ゼロで disabled)/ Sort by(サブメニュー)/ セパレーター /
    **Keep on Top**(トグル。チェックマークで現在値を示す。下記)/
    Close Window(⌘W。key ウィンドウを閉じる標準動作)/
    **Conversations…**(⌄ メニューの同名項目と同一。§4.6)/ Quit Shiibar CC(⌘Q)。
  ...
  - ドロップダウンの置き換えではない(どちらも存在し、どちらからでも一覧が見られる)。
    Open as Window を押すとドロップダウンは閉じる(Settings… 等と同じ「移った先に仕事を渡した」流儀)
- **Settings ウィンドウ**(⌄ → Settings…): 設定をまとめる独立ウィンドウ。Setup Check と同じく
  ...
  囲みカード・行区切りは OS が描く。素の VStack + Toggle + Divider にはしない)。構成は 2 グループ:
...
- **索引の内容**: **全会話**(実行中も含む)+ **live/past フラグ**。絞り込みは消費者の責任。
  ...
  ユーザープロンプト先頭 → それも無ければ null**。フォルダラベルへの最終フォールバックは表示側の
  責任。§7-6)/ cwd / 最終更新(transcript の mtime)/ 発話列(出現順 + 役割 + 全文)。
...
- **アクティブ経路**: 葉 = **ファイル内で最後に現れる、完全な・sidechain でない user / assistant 行**
  ...
  新しい枝の追記なので同じ。§7-6)。そこから `parentUuid` を遡って集める。親が見つからない場合は
...
- **live/past フラグ**: `~/.claude/sessions/<pid>.json`(§7-6)を読み、**`kill(pid, 0)` による生存 +
  プロセス名が `claude`** の 2 段チェックを通ったものの session_id を live とする。pid の再利用は
...
- **検索**(`conversations search <query>`): 追いつき後に検索。クエリは**前後の空白(半角・全角)を
  ...
  各語は大文字小文字無視の単純部分一致(アクセント同一視なし — §7-6)。**語は 2 文字から** —
...
- **入り口**: ⌄ メニューとアプリメニューの `Conversations…`(§4.5。最終グループ、セパレーター
  追加なし)。窓が存在する間は disabled(Open as Window と同じ規約)。窓が存在する間だけ通常アプリに
  なる規則(§8.30)はこの窓にも適用する
- **ウィンドウ**: 2 ペイン(左 = 検索 + 一覧 + ステータス行、右 = 会話の全文 + Resume)。
  クロームは一覧ウィンドウと同じタイトルバー非表示 + 信号機の帯(`title` は **Conversations**)。
  縦横ともリサイズ可で、**サイズ・位置を記憶する**(一覧ウィンドウと違いアイコン直下の定位置規則は
  持たない — アイコン起点の画面ではないため)。閉じるのは赤ボタンと ⌘W
- **一覧(左)**: `conversations search` の結果を**全部**出す(実行中の会話も含む)。新しい順・
  区切りなし。行は 2 行 — **1 行目 = タイトル**(null はフォルダラベルに落とす)/ **2 行目 =
  フォルダラベル + 経過時間**(ラベルは §4.5 と同じ末尾 2 要素規則)。**実行中の会話は 2 行目に
  `running` の淡いテキスト印**(Agents ビューの状態記号は使わない — この印の鮮度は最後の検索
  実行時点であり、いまの状態を主張しない)。live の行に Resume は出さない(アクションなし —
  実行中の会話への動詞は実感が出たら設計する)
- **検索欄(左上)**: インクリメンタル。デバウンス(§9)後に `conversations search --json` を
  subprocess 実行し、進行中の実行はキャンセルして新しいクエリを投げる。**trim 後に有効な語
...
- **会話の表示(右)**: 行を選ぶと `conversations show` で**全文を 1 回で取得**し、古い順に表示
  ...
  **ヒットナビゲーション**: show 取得後、アプリが手元の全文から**各語の全出現**を計算し
  (意味論は CLI と同じ部分一致)、**本文に 1 件以上あるときだけ**会話の上部に検索バー行
  (文書順のカウンタ + ▲ = 古い方へ / ▼ = 新しい方へ)を出して全語を同色でハイライト
...
- **表示の状態規則**: 一覧の更新はプレビューに触らない(一覧とプレビューは別の状態。選択は
  ...
  **500 文字(§9)を超えるメッセージは畳んで `Show full message` で展開**。畳まれた部分への
  ヒットもカウンタに数え、▲▼ でジャンプしたら自動で展開する
- **Resume ボタン**(past の行のみ): `shiibar-cc resume --cwd <cwd> <session_id>` を subprocess
...
6. **transcript(会話ファイル)の構造** — 一部実測済み(2026-07-12、手元の実ファイル 300 件)。
   残りは M34 実装時に実物で確定する:
   ...
   - `user` / `assistant` 行は `uuid` / `parentUuid` を持つ(実測 2026-07-12)— **ファイルは追記専用の木**で、
     ...
     行に `isSidechain` フィールドあり(サブエージェントの会話の分離マークと推定)
   ...
   - `~/.claude/sessions/<pid>.json`(§7-2)は実行中プロセスと 1:1 を実測(2026-07-12、
     2 ファイル vs 実行中 claude 2 プロセス。古いファイルの残留なし。プロセス名は `claude` — `ps` 実測)
   ...
   - **要確認(M34 実装時)**: user 行のうち「人の発話」と tool_result・スラッシュコマンド出力の弁別規則 /
     assistant 行の text block 抽出規則 / タイトル規則の検証(`custom-title` と `ai-title` の共存時の
     更新のされ方・欠落の頻度)/ **葉の決定規則の検証**(「最後の完全な非 sidechain 行」と
     `leafUuid` の答え合わせ。rewind 直後・実行中の書きかけ末尾での挙動)/ `isSidechain` の実際の意味 /
     `sessions/<pid>.json` のライフサイクル(開始時にすぐ現れるか・正常終了で消えるか・クラッシュでの残り方)
...
- **抽出はアクティブ経路のみ・変更ファイルは全読み直し**: 現在の葉から `parentUuid` 遡行 —
  ...
  最後の完全な非 sidechain 行」**(leafUuid が最後のユーザー入力を指す場合、素直に遡ると最後の回答が
...
| Conversations 窓の初期サイズ | 640×480pt(左ペイン 280pt)。縦横リサイズ可・サイズと位置を記憶 | 固定 |
| メッセージの畳み             | 先頭 500 文字を超えたら畳む(`Show full message` で展開。DB は全文) | 固定 |
