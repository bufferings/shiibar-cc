...
- **ヒットナビゲーション**: show 取得後、アプリが**描画後のテキスト**(Markdown 記号を消費した後の、
  ...
  **本文に 1 件以上あるときだけ**会話の上部に検索バー行
  (文書順のカウンタ + ▲ = 古い方へ / ▼ = 新しい方へ)を出して全語を同色でハイライト
  (現在位置のみ濃く)、初期位置は**どれかの語の最新の出現**。0 件なら検索バー行を出さず下端から。
  クエリを打ち替えたらハイライトとカウンタは再計算するが**スクロールは動かさない**。
  **ヒット目盛り**: 検索バー行が出ている間、右ペインの右端に会話全体のヒット分布の目盛りを重ねる
  (現在位置だけ濃く。位置はヒットを含むメッセージ単位の近似でよい — 1 メッセージ内の複数ヒットの
  目盛りは重なってよい)
- **表示の状態規則**: 一覧の更新はプレビューに触らない(一覧とプレビューは別の状態。選択は
  ...
  **500 文字(§9。描画後テキストで数える)を超えるメッセージは畳んで `Show full message` で展開**。
  ...
  畳まれた部分へのヒットもカウンタに数え、▲▼ でジャンプしたら自動で展開する。
...
- **描画エンジン(右ペインのメッセージ面)**: アプリ同梱のページを表示する WKWebView(システム
  ...
  ヒット目盛りの縦位置だけは実レイアウト由来(§4.6 の近似規定を満たす)
- **選択とコピー**: メッセージをまたぐ連続選択ができる(文書として扱う)。⏺ マーカー・
  展開/畳みボタンなどの操作部品は選択・コピーに含めない。右クリックメニューは
  **Copy(⌘C 併記)/ Copy as Markdown(⇧⌘C 併記)の 2 項目だけ**(Copy は WebKit 標準の
  項目を流用 — 自前で複製しない。Look Up 等それ以外の標準項目は取り除く)。
  **⌘C / ⇧⌘C はペイン自身がキーを受けて処理する** — このアプリは Edit メニューを持たない
  (§4.5)ため、メニュー経由のキー配達が存在しない(§8.38)。Copy = 選択部分を表示どおりに。
  **Copy as Markdown = 選択範囲を、描画に使ったブロック構造とインラインスタイルから Markdown に
  直列化**したもの(見出し・リスト・コードブロック・表と、インラインの code・強調・打ち消し・
  リンクを復元する。原文の文字列一致ではなく「選択した部分の Markdown 表現」)。選択が無いときは
  カーソル下のメッセージの**原文**を丸ごと、カーソルがメッセージ外なら何もしない。
  コピー動線(ショートカット押下 → ペーストボード)は端到端の自動テストを持つ(§8.38)
...
- **実機スモークからの手直し**(2026-07-14、所有者): (1) メニューの Copy は **WebKit 標準の
  ...
  ペーストボードまでの端到端テストを必須にする(§4.6)。合わせてメニューは所有者判断で
...
| Conversations 窓の初期サイズ | 640×480pt(サイドバー 250pt)。縦横リサイズ可・サイズと位置を記憶 | 固定 |
