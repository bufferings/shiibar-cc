...
- 状態ディレクトリ: `~/.local/state/shiibar-cc/`(起動時に 0700 で作成)。
  ...
  中身: `shiibar-ccd.sock` / `state.json` / `sessions.jsonl` / `shiibar-ccd.log`
...
- **resume**: fzf があれば fzf、なければ番号選択のプロンプト。実行中(list に存在する)セッションは候補から除外する
  (`claude --resume` の二重起動防止)。さらに **Claude Code の会話ファイル
  `~/.claude/projects/<cwd の slug>/<session_id>.jsonl` が存在するものに絞る**(slug = cwd の `/` を `-` に
  置換。2026-07-05 実機で確認した配置。サブエージェントのセッションは hook で履歴に載るが会話ファイルを
  持たず resume 不可 — §7-3)。**プロジェクトディレクトリが解決できない場合(該当ディレクトリが存在しない/
  ディレクトリでない)は絞らない(fail-open)** —
  Claude 側の配置変更で候補が全滅するより、選んで失敗するほうがまし。
  履歴の cwd が消えていれば警告して `$HOME` で開く。
  候補ゼロ(履歴なし・全員実行中・会話ファイルなし)と選択の中断(fzf の abort / プロンプトの EOF)は
  「該当なし」= exit 2(理由は stderr)。Ctrl-C(SIGINT)は握らず POSIX 標準のまま(130)
...
| M   | 内容                                               | 自動テスト                                                   | 実機スモーク                                   |
| --- | -------------------------------------------------- | ------------------------------------------------------------ | ---------------------------------------------- |
| M1  | proto + shiibar-ccd(report/list/subscribe/remove/seen/sessions/info/shutdown、state.json 復元、ログ)+ `shiibar-cc report` + hooks | 遷移表(§3.4)のテーブル駆動テスト。fixtures 再生 → subscribe 出力列が期待と一致。daemon 再起動で状態復元 | 実セッションの遷移を watch で観測。実 hook JSON を fixtures/ に採取 |
...
3. **hook イベントのペイロード仕様** — ✅ 公式ドキュメント確認済み(2026-07-04)。Notification は `notification_type` で 8 種、
   ...
   **`waiting` は実「許可プロンプト」でも出る**(2026-07-04 実機スモークで確認: daemon 不在中に
   ...
   幽霊 prune も実機確認)。
   実機確認済み(2026-07-05): **許可プロンプトの Esc 拒否では hook が一切発火しない**(PostToolUseFailure も
   その後の Stop も来ない — waiting が残り、reconcile が backstop する実例)。
   **サブエージェント(Task tool)のセッションは `$ITERM_SESSION_ID` を継承して hook を発火する** —
   親と同じ target・自分の session_id で報告され、sessions 履歴に載る。ただし会話ファイル
   (`~/.claude/projects/<slug>/<session_id>.jsonl`)を持たず `claude --resume` 不可(resume の候補絞りの根拠。§4.4)。
   **残検証**: (a) elicitation_* の実際の意味、(b) `PostToolUseFailure` の実発火(ツール実行の実失敗時。
   ユーザーの Esc 拒否で発火しないことは確認済み)、(c) サブエージェントの hook が親と同じ target を
   上書きすることによる表示への影響(例: サブエージェントの Stop で親エントリが一時的に idle に見える可能性。
   §3.6 の target 主キー設計の帰結 — 実害の有無をドッグフーディングで観察)
...
resume(M3)・遅延通知・前面抑制は当初から意図した機能であり、v1 に残す。
