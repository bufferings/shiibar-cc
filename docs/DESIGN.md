...
- **検索欄(左上)**: インクリメンタル。デバウンス(§9)後に `conversations search --json` を
  ...
  **押下には必ず見える反応がある** — ボタン自体が状態を持つ(§8.42):
  ホバーで浮く / 押下で沈む / **実行中は矢印グリフが回転**して disabled(この回転は
  リロードボタンの標準文法で、working のグリフ循環スピナー ✻ とは別物)。
  検索は数十 ms で終わるため、**実行が一瞬でも最低 1 回転(§9)は回してから**通常状態に戻る
  (回転が知覚できる長さを保証する — §8.43)。あわせて実行中は
...
| Conversations ⟳ の最低回転  | 1 回転(0.8 秒)— 実行が先に終わっても回り切ってから通常状態へ | 固定 |
