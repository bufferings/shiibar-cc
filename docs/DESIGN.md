...
- **選択とコピー**: メッセージをまたぐ連続選択ができる(文書として扱う)。⏺ マーカー・
  ...
  **Copy(⌘C)/ Copy as Markdown(⇧⌘C)** の 2 項目(ショートカット併記)。
  Copy = 選択部分を表示どおりに、Copy as Markdown = **選択が触れているメッセージの原文
  (Markdown)を丸ごと**。選択が無いときは Copy は disabled、Copy as Markdown はカーソル下の
  メッセージの原文。⌘C / ⇧⌘C は選択があればメニューを開かずに直接効く
...
- **採用に対する反論も記録する**: +100MB のプロセスと、自分で持ち込んだ分のセキュリティ面、
  ...
  実装の乖離が構造的に消えること
- コピーの文法: 右クリック = **Copy(⌘C)/ Copy as Markdown(⇧⌘C)** の 2 項目(§4.6)。
