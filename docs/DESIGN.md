...
「—」は状態変更なし(report の場合は last_seen と session_id / cwd の上書きは行われる。§3.2)。
「登録(x)」は未登録 target のエントリ新規作成。
seen は report ではないため、done → idle 以外の場合は効果ゼロ(last_seen も更新しない)。
...
- **欠損・未知フィールドの既定**: `notification_type` を欠く Notification は未知種別と同じ(blocked に倒す)。
  `source` を欠く/未知の値の SessionStart は startup と同じ(idle)。
  `background_tasks` フィールド自体を欠く Stop は空配列と同じ(done)
...
- エントリ属性: `target`(プライマリキー)/ `status` / `session_id` / `cwd` / `since`(現在の状態に入った時刻)
  ...
  同一 target への report の上書き規則(§7-3): `session_id` / `cwd` は毎回上書き。
  `task` は prompt を運ぶ report(UserPromptSubmit)のときのみ上書き。
  `message` は blocked への遷移/維持を実際に起こす Notification のときのみ上書き
  (no-op の Notification は message を持っていても無視する)
...
- **手動削除**: `shiibar-cc remove <selector>` で幽霊エントリ(SessionEnd 取りこぼし等)を消せる(§4.4)
...
- **状態スナップショット**: 状態が実際に変化したとき(report は **last_seen のみの更新も変化に含む**。
  ...
  全状態を `state.json` へ atomic に上書き(tmp + rename)。起動時に読み込んで復元する(daemon 再起動で blocked の存在が消えないこと。
...
shiibar-cc wait <selector> --status done|blocked|idle|working [--timeout SEC]
...
shiibar-cc remove <selector>  # 幽霊エントリの削除
...
| M   | 内容                                               | 自動テスト                                                   | 実機スモーク                                   |
| --- | -------------------------------------------------- | ------------------------------------------------------------ | ---------------------------------------------- |
| M1  | proto + shiibar-ccd(report/list/subscribe/remove/seen/info/shutdown、state.json 復元、ログ)+ `shiibar-cc report` + hooks | 遷移表(§3.1)のテーブル駆動テスト。fixtures 再生 → subscribe 出力列が期待と一致。daemon 再起動で状態復元 | 実セッションの遷移を watch で観測。実 hook JSON を fixtures/ に採取 |
| M2  | shiibar-cc-client + shiibar-cc(list/watch/wait/focus/focused/remove/doctor)+ install.sh(バイナリ + hooks。daemon は手動起動、.app は M4) | exit code 系統(wait: 0/1/2/124、focus: 0/1/2/3)。selector 解決。iterm モジュールの純関数部 | `wait --status done && say done` が動く。focus で該当タブが前面に来る。doctor が全項目 green |
...
2. **hook イベントのペイロード仕様** — ✅ 公式ドキュメントで確認済み(2026-07-04):
   ...
   **M1 の残検証**: 実ログとの突き合わせ。特に (a) `idle_prompt` の発火条件と対象状態、
   ...
   (d) `PostToolUseFailure` hook が実在・発火するか(実装エージェントから疑義の申告あり。
   発火しない場合でも「権限拒否 → いずれ Stop で done」で大きくは壊れないが、blocked 解除が遅れる)
