...
set / clear は **status の遷移で決まる**(hook でも reconcile でも同じ規則):

- **立てる**: status が **`waiting` になった**とき / **完了で `idle` になった**とき(Stop・background_tasks 空)
  / reconcile が未追跡セッションを `idle`・`waiting` で新発見したとき(§3.5)
- **下ろす**: focus した(= seen)/ status が **`working` になった**とき / SessionStart(新規開始)
- **触らない**: status が変わらないとき(既に立っている `waiting` を reconcile が再確認しても保持)

「立てる」が「入ったとき(遷移)」なのが要点: 既に追跡している既知セッションでも、`waiting` へ遷移すれば
(hook を取りこぼして reconcile で気づいた場合も含め)未確認として通知される。
...
3. **突き合わせ(キー = target)**:
   ...
   - 既知 → status を反映(claude agents が常に勝つ。時刻での調停はしない。§3.3)。
     status が **`waiting` / `idle` へ遷移したら §3.2 の set ルールを適用**(`waiting` になった → 立てる。
     ただし既知セッションが `idle` へ遷移した場合は立てない = 完了は claude agents から判別できない。§8.12)。
     status が不変なら flag は触らず、`last_seen` を更新する(生きたエントリを stale sweep で消さないため)。
     `waiting` の場合は `message` を `waitingFor` で更新する
...
- 例外は「← 戻る」(focus の一種)と「再スキャン」(= reconcile。現実に合わせて表示を直すリフレッシュ/自己修復であり、確認フローの要る破壊的操作ではない)。どちらも読み取り+整えるの範囲内
- **判断基準の一般形**: 迷ったら、破壊的・対話的なら CLI、読み取り・ジャンプ・リフレッシュならメニューバー
...
resume(M3)・遅延通知・前面抑制は原設計の意図的な機能であり、`focus -` は実装が十分小さい(ファイル 1 つ)ため、いずれも v1 に残す。
