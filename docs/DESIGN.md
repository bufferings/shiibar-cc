...
- **target は不透明文字列**。中身は `$ITERM_SESSION_ID` だが、shiibar-ccd は解釈せず保持・転送するだけ。
...
- 状態ディレクトリ: `~/.local/state/shiibar-cc/`(起動時に 0700 で作成)。
  ...
  中身: `shiibar-ccd.sock` / `state.json` / `sessions.jsonl` / `shiibar-ccd.log` / `last_focus`
...
- 旧 spec の `blocked` は `waiting` に改名(claude agents(§3.3)の語彙と揃える)。同義
- 旧 spec の `done`(完了直後)は status ではなくなり、`idle` + unreviewed(§3.2)として表す

### 3.2 unreviewed フラグ

「そのセッションが**あなたの番の状態に入ってから、まだ focus していない**」ことを表す。
**`waiting` と `idle` にのみ乗る**(`working` はあなたの番ではないので持たない)。

| status + flag           | 意味                                                 |
| ----------------------- | ---------------------------------------------------- |
| `waiting` + unreviewed  | 新しい許可待ち、未確認 ← **最重要アラート**            |
| `waiting`(確認済)       | 許可待ちだが、既にそのペインに行った(対応中)         |
| `idle` + unreviewed     | 完了して結果を未確認(= 旧 done)                     |
| `idle`(確認済)          | 見た/そもそも何もしていない(静か)                   |

set / clear:

- **立てる**: `waiting` に入ったとき / タスク完了で `idle` になったとき(Stop・background_tasks 空)
  / **reconcile で未追跡セッションを `idle`・`waiting` として新発見したとき**(§3.5)
- **下ろす**: focus した(= seen)/ `working` に移った
- **触らない**: status が変わらないとき(reconcile が既知セッションを再確認したとき等は保持)

### 3.3 status の source(2 つ)

- **hooks(リアルタイム push・主軸)**: イベントを受けて即座に status を更新(§3.4)
- **`claude agents --json`(reconcile・補正)**: 起動時 + 定期ポーリング + 手動リロードで実行。
  権威ある snapshot で drift を直す(§3.5)。hooks が主でリアルタイム、reconcile が backstop
...
| seen(focus 成功時に client が送る。§4.4)                                                     | 無視          | —       | 下ろす  | 下ろす  | 下ろす       |
...
- **Notification 分岐**: 未知種別は waiting に倒す(見逃しより誤報を許容)。
  ...
  reconcile が backstop になるため、旧 spec の「working→blocked」ハックは廃止した
- **PostToolUse は waiting 解除用**: 並行ツール実行による早期解除のレースは既知の制約として許容(§8.7)
...
- **同値セル**: `since` は status が変わったときのみ更新。同値遷移は「—」と観測上同じ

### 3.5 reconcile(claude agents ベース)

`claude agents --json` の各セッションについて(gather は client 側、適用は daemon の `reconcile` コマンド。§4.2):

1. **target を導出**: `pid → tty(`ps`)→ iTerm2 セッションの UUID`(§7-1)。
   iTerm2 に一致しなければ **skip**(iTerm2 外は追跡しない。§8.11)
2. **status をマップ**: `busy` / `shell` → working、`waiting` → waiting、`idle` → idle
3. **突き合わせ(キー = target)**:
   - 未追跡 → **追加**。`idle` / `waiting` なら **unreviewed を立てる**(新発見 = あなたが未確認)
   - 既知 → status を更新。ただし **claude agents の `statusUpdatedAt` が daemon の最終更新より新しいときだけ**反映
     (hook のより新しい更新を潰さない = 時刻ベースの last-write-wins)。**unreviewed は触らない**
     (claude agents は確認済/未確認を知らない)
4. **prune**: claude agents に居ない(target が live 集合に無い)エントリを削除し `agent_removed` を配信

reconcile は claude agents(Claude 自身の権威ある登録簿)を source にするので、
「居ない = 本当に死んでいる」が信頼でき、生きた `waiting` を誤って消すことはない(必ず一覧に載る)。
`waiting` の新発見に unreviewed が付く = **daemon 不在中に許可待ちになったセッションもここで拾える**。
唯一 reconcile で復元できないのは「完了(旧 done)」= 完了時の unreviewed だけ(claude agents は idle としか見えない。§8.12)。
...
- 上書き規則(同一 target への report。§7-3): `session_id` / `cwd` は毎回。`task` は prompt を運ぶ report のみ。
  `message` は waiting への遷移/維持を起こす Notification のみ(waiting 以外に移ったら消す)
- **時刻はすべて daemon の時計**(epoch 秒)。report の `ts` は表示用。reconcile 時のみ claude agents の
  `statusUpdatedAt` と比較する。daemon 内の時計は注入可能にする(stale テストのため)
...
- **手動削除**: `shiibar-cc remove <selector>`(reconcile が backstop になったので通常は不要だが残す)
...
- **target の生成規則**: `$ITERM_SESSION_ID` があればその値を target にする。
...
// daemon が add / update(statusUpdatedAt で last-write-wins)/ prune / flag を適用する
{"cmd":"reconcile","sessions":[
  {"target":"…","session_id":"…","cwd":"…","status":"waiting","status_updated_at":…,"waiting_for":"permission prompt"}
]}  // → {"ok":true}
...
- `shiibar-cc-client`: socket 接続、list / subscribe / wait のクライアント実装と、**iterm モジュール**
  ...
  - iterm モジュール(iTerm2 の知識はここにのみ存在させる)。
    **テスト分離**: AppleScript ソースの生成と osascript 出力のパースは純関数として切り出し、
    ...
    - `focused()`: iTerm2 が最前面アプリのとき、前面 session の UUID を `w0t0p0:UUID` 形で返す。前面でなければ「なし」
    - `open_tab(cwd, cmd)`: 新規タブを cwd で開いてコマンドを実行(resume 用)。こちらは iTerm2 を起動してよい
    - `iterm_targets()`: **reconcile 用**。`ps` で claude の pid → tty を引き、iTerm2 の各セッションの tty と
      突き合わせて `{pid → target(UUID)}` を返す(§3.5 / §7-1)。iTerm2 に無い pid は返さない(iTerm2 外 = 追跡外)
...
shiibar-cc focus <selector>   # ジャンプ。成功時: 移動前の前面 target を last_focus に保存し、daemon に seen を送る
shiibar-cc focus -            # last_focus に戻る(ジャンプ→対応→元の作業へ、の帰り道)。
...
shiibar-cc resume             # sessions から選択 → 履歴の cwd で新規 iTerm2 タブを開き claude --resume <id>
...
- **見た目の正は `docs/menubar-design.html`**。トレイは塗りなしの円形枠線 + 上下左右ランプの点灯/消灯式。
  status(working / waiting / idle)3 色と unreviewed フラグ(§3)を**どう表現するかは menubar-design.html で再議論中**
  (旧「done」= idle + unreviewed の見せ方を含む)。描画は NSImage 合成で状態変化ごとに描き直す。本節は挙動のみを規定する
...
- **デスクトップ通知**: `UNUserNotificationCenter`。**unreviewed フラグが立ったとき**(waiting に入った / 完了で idle+unreviewed)に発火。
  クリックは delegate で受けて focus。「あなたの番になった」= unreviewed の立ち上がりが通知の起点なので、状態レイヤーと一致する
  - 遅延通知: 3 秒後にタイマー発火した時点で**最新状態を再確認**して、まだ unreviewed なら通知(スリープ跨ぎでも安全)
  - **通知済み管理**: アプリは (target, since) 単位で通知済みを記録する。再接続時の snapshot / reconcile 経由の
    unreviewed も通知対象(切断中の遷移を取りこぼさない)だが、通知済みの同一状態には再通知しない
  - 前面抑制: 通知直前に `shiibar-cc focused` を叩き、対象 target が前面ならその通知は出さない
  - 通知の掃除: unreviewed が下りたとき・`agent_removed`・focus 時に該当 target の配信済み通知を `removeDeliveredNotifications` で消す
...
- **reconcile の実行**: アプリは `shiibar-cc reconcile` を **(1) 起動時 / daemon 接続時、(2) 定期(§9 の間隔)、
  (3) 再スキャンボタン** で呼ぶ(§3.5)。これで daemon 不在中の取りこぼし(幽霊・見逃した waiting)を
  status レベルで自己修復する。hooks 主軸のリアルタイム性は保ったまま backstop になる
...
| M   | 内容                                               | 自動テスト                                                   | 実機スモーク                                   |
| --- | -------------------------------------------------- | ------------------------------------------------------------ | ---------------------------------------------- |
| M1  | proto + shiibar-ccd(report/list/subscribe/remove/seen/info/shutdown、state.json 復元、ログ)+ `shiibar-cc report` + hooks | 遷移表(§3.4)のテーブル駆動テスト。fixtures 再生 → subscribe 出力列が期待と一致。daemon 再起動で状態復元 | 実セッションの遷移を watch で観測。実 hook JSON を fixtures/ に採取 |
...
| M3  | resume(sessions 保存 + `shiibar-cc resume`)        | sessions.jsonl の重複排除・compaction。実行中セッションの除外 | daemon 再起動後も履歴から `claude --resume` で復帰できる |
| M4  | メニューバーアプリ                                 | (UI は手動中心)                                              | アプリ起動で daemon 起動・既存 daemon へのアタッチ・Quit で daemon 停止・起動時/定期/手動 reconcile。ロールアップ表示・ドロップダウン focus・「戻る」・「再スキャン」。unreviewed 通知(遅延・前面抑制: waiting 中に該当タブへフォーカスして通知が出ないこと)。focus で配信済み通知が消えること。切断中・権限 denied の警告行 |
...
2. **claude agents / セッション状態の取得** — ✅ 実機で検証済み(2026-07-04):
   ...
   - **session status は 4 値**: `busy` / `shell` / `idle` / `waiting`。実測: 処理中 = `busy`、入力待ち = `idle`、
     許可プロンプト表示中 = `waiting`(さらに `waitingFor: "permission prompt"` が付く)。→ **blocked が取れる**
   - reconcile の status マップ: busy/shell → working、waiting → waiting、idle → idle(§3.5)
   - **done は取れない**(claude agents では idle と同じ)= reconcile で復元できない唯一の状態(§8.12)
3. **hook イベントのペイロード仕様** — ✅ 公式ドキュメント確認済み(2026-07-04)。Notification は `notification_type` で 8 種、
   ...
   **残検証**: (a) elicitation_* の実際の意味、(b) `PostToolUseFailure` の実発火、(c) `waiting` が
   AskUserQuestion 以外の実「許可プロンプト」でも出るか(強い状況証拠あり、100% は要実確認)
...
- 定数は §9 の表に集約し、v1 の変更手段は環境変数 2 つ(`SHIIBAR_CC_STATE_DIR` / `SHIIBAR_CC_LOG`)のみ。
...
- 旧 §4.1 の `session:<session_id>` フォールバック target は廃止。`$ITERM_SESSION_ID` が無い report は drop、reconcile は pid→tty が iTerm2 に一致しないセッションを skip する。結果 daemon が持つのは常に iTerm2 セッションだけ = 全部 focus できる
- 「空 target 衝突を防ぐためフォールバックが要る」は**追跡する前提**の話だった。追跡しないなら衝突も起きず、フォールバックの存在理由自体が消える
- **再検討の条件**: iTerm2 以外のターミナルに focus 対応する PR を書くとき(そのとき target 生成規則ごと見直す)

### 8.12 reconcile で復元できないのは done(idle+unreviewed)だけ、受け入れる

- daemon 不在中の取りこぼしは、次の reconcile(§3.5)でほぼ全部 status レベルで復元できる(waiting すら claude agents から拾える)
- 唯一の例外が「完了直後(旧 done)」= `idle` + unreviewed。claude agents は done と idle を区別しないため、reconcile では未確認マークを立てられず idle で入る
- ただし done の**通知は完了時(hook の Stop)に既に飛んでいる**ので、reconcile を跨いで失うのは「未確認マークの残り」だけ。一過性の状態であり実害は小さい。これは「穴があっても平気な作り」の許容範囲として受け入れる
- **再検討の条件**: この「跨いだ done が消える」が体感で問題になったとき(まず起きない見込み)
...
- 理由: shiibar の状態語彙は「ユーザーが何をすべきか(注意・行動)」で定義する。`shell` は「働いている」以上の注意差が無いので working に潰す。逆に「完了して未確認(旧 done)」は idle と注意差があるので unreviewed フラグで残す。claude が done を idle と同一視するのは Claude の関心軸が違うだけ
