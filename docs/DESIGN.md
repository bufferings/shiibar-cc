...
- **一覧ウィンドウ**(⌄ → Agents…): ドロップダウンと同じ一覧を、閉じるまで出しっぱなしに
  ...
  - **メニューはアプリメニューと Edit メニューの 2 個**(File / Format / View / Window / Help は
    ...
    **システムが Edit に注入する項目(AutoFill / Start Dictation / Emoji & Symbols)は出さない**
    (Dictation・絵文字は標準の抑止設定、AutoFill はメニュー整形時に非表示 — §8.42)。
    アプリメニューの構成:
...
- **検索欄(左上)**: インクリメンタル。デバウンス(§9)後に `conversations search --json` を
  ...
  **押下には必ず見える反応がある** — ボタン自体が状態を持つ(§8.42):
  ...
  リロードボタンの標準文法で、working のグリフ循環スピナー ✻ とは別物)。あわせて実行中は
...
### 8.42 ボタンは自分で反応する(⟳・Resume)。Edit の注入項目は出さない(M39)
...
- **Edit メニューのシステム注入項目(AutoFill / Start Dictation / Emoji & Symbols)を出さない**
  (所有者: いらない)。Dictation・絵文字は macOS 標準の抑止設定、AutoFill は
  MainMenuPruner と同じ「隠す」方式で整形時に非表示にする
