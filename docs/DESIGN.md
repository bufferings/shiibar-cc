...
- **並び替え**(⌄ メニュー / アプリメニューの「Sort by」ラジオ 2 択。UserDefaults 永続
  ...
  行が動かないため。§8.31)
- 最上部の **⌄ メニュー**の構成(並び順は一覧ウィンドウ表示中のアプリメニュー(下記)と同一 —
  最終グループの「窓の動詞」だけが器で入れ替わる。§8.30):
  ...
  **Open as Window**(一覧ウィンドウを開く。下記。アプリメニューの Close Window に当たる位置。
  ⌄ 固有の項目 — 一覧ウィンドウ自体には ⌄ が無い。**窓が存在する間は disabled**)/ **Quit**
...
- **一覧ウィンドウ**(⌄ → Open as Window): ドロップダウンと同じ一覧を、閉じるまで出しっぱなしに
  ...
  - **タイトルバーの無い普通のウィンドウ**: タイトルバーは非表示(SwiftUI の `hiddenTitleBar`)。
    ...
    画面のクロームには描かれない)。
    窓としての性質は普通のウィンドウ: 最前面固定しない・全 Space に追従しない・
    よそをクリックしても閉じない・信号機の帯を掴んでドラッグできる。
    閉じるのは赤ボタンと ⌘W。リサイズ不可(幅の規定はドロップダウンと同じ。
    高さは一覧の自然な高さ + 帯)。
  ...
  - **窓が存在する間だけ、アプリは通常アプリになる**(§8.30): 窓が開くと Dock と ⌘Tab に載り、
    ...
    Settings / Setup Check を単独で開いても切り替えは起きない(条件はあくまで一覧ウィンドウ)。
  ...
  - **アプリメニューは 1 個だけ**(File / Edit / Window メニューは出さない):
    ...
    Close Window(⌘W。key ウィンドウを閉じる標準動作)/ Quit Shiibar CC(⌘Q)。
    各項目の挙動は ⌄ メニューの同名項目と同一
