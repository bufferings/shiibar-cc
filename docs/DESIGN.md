...
- 属性: `target`(プライマリキー)/ `status` / `unreviewed` / `session_id` / `cwd`
  ...
  / `last_report_at`(最後に hook report を受けた時刻 — Recent activity 並びのキー。§4.5。
...
- **配布は Claude Code のプラグイン機構**で行う(§8.19)。リポジトリ自体がマーケットプレイスを兼ねる:
  ...
  - `plugin/.claude-plugin/plugin.json`: プラグインマニフェスト(name / description / version。
    version は明示し、hooks 構成を変えたら上げる — 利用者への配布はこの bump で制御される)
  ...
  - 導入は `/plugin marketplace add bufferings/shiibar-cc` → `/plugin install shiibar-cc@shiibar-cc`
    の 2 コマンド。`~/.claude/settings.json` は手編集しない(プラグインの hooks はユーザー設定と
    自動マージされ、導入の記録は Claude Code が `enabledPlugins` に書く)。撤去は `/plugin uninstall shiibar-cc`
...
//                                                 配信頻度は上がるが、並びの確定は開いた時点なので行は動かない。§4.5)
...
- **見た目の正は `docs/menubar-design.html`**(2026-07-05 確定)。トレイ = フルハイトの角丸窓 +
  左上の **✳ 紋章**(素の U+2733)+ 右下の状態表示、の**テンプレートアイコン**(「Claude が居る窓」)。
  状態表示は waiting `!` > working **点灯式ドット(4 コマアニメーション。§9)** > idle **無印** のロールアップ。
  減光 2 段階: idle 0.8 / エージェントなし・切断 0.45(✳ と枠のみ)。unreviewed が 1 台でもいれば枠の角に赤バッジ。
  描画は NSImage 合成で状態変化ごとに描き直し、**タイマーは working 表示のときだけ**動かす
  (点灯アニメは透明度のみの変化なのでテンプレート画像のまま成立する)。本節は挙動のみを規定する
...
- 最上部の **⌄ メニュー**の構成(セパレーターで 3 グループ。§8.25):
  ...
  - **表示と設定**: 並び替え(Sort by、サブメニュー)/ **Settings…**(設定ウィンドウを開く。
    下記・§8.26)/ **Setup Check…**
...
- **reconcile の実行**: アプリは `shiibar-cc reconcile` を **(1) 起動時 / daemon 再接続時、(2) 定期
  ...
  スケジュールを止め、復帰後に自然に再開する — 自前の画面状態検知はしない)、(3) ドロップダウン
  ⌄ メニューの「再スキャン(Rescan)」** で呼ぶ(§3.5/§8.22)。これで hooks の取りこぼしによる
...
│       └── report.sh
├── scripts/
│   ├── install.sh          # バイナリ配置 + .app 化(shiibar-ccd・shiibar-cc 同梱)+ CLI symlink + Login Items + プラグイン導入案内
│   ├── uninstall.sh
