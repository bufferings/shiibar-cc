...
3. **hook イベントのペイロード仕様** — ✅ 公式ドキュメント確認済み(2026-07-04)。Notification は `notification_type` で 8 種、
   ...
   **AskUserQuestion のダイアログは Notification を発火しない**(表示中も status は working のまま =
   「あなたの番」を取りこぼす既知の穴。backstop は reconcile)。
   **別プロセスで起動されるエージェントセッション(コーディネータが spawn する類)は `$ITERM_SESSION_ID` を
   ...
   subscribe ストリームで実観測)。
   **残検証**: (a) elicitation_* の発火条件(AskUserQuestion では発火しないことを確認済み。
   MCP の elicitation ダイアログと推定 — 未観測)、(b) spawn 型エージェントセッションの hook(Stop 等)が
   親と同じ target を上書きすることによる表示への実害の有無(会話内 Task は無発火と確認済みだが、
   spawn 型は発火する — ドッグフーディングで観察)
