...
- **Jump ボタン**(live の行のみ — 上記の 3 状態。§8.48): 選択中の会話の session_id を Agents
  一覧から引き、一致したエントリの target へ focus する。経路は §4.5 の行クリックと**同一**
  (`shiibar-cc focus <target>` の subprocess・exit 2/3 の扱い・成功時の seen 送信まで共通)。
  ウィンドウは閉じない(focus でターミナルが前面に来るので、閉じる操作を足す理由がない —
  Resume と同じ)。**Jump 成功後に search の再実行はしない**(Resume と違い、会話の状態は
  何も変わらないため)。行の running 印(最後の検索実行時点)とボタン状態(選択時点)の
  鮮度のズレは防がない — 押した瞬間にタブが閉じていれば focus の「該当なし」が出るだけ
  (§8.34 の割り切り。二重の防御はしない)
