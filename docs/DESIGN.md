...
- **選択とコピー**: メッセージをまたぐ連続選択ができる(文書として扱う)。⏺ マーカー・
  展開/畳みボタンなどの操作部品は選択・コピーに含めない。右クリックメニューは WebKit 標準の
  項目(Look Up 等)を残したまま、**Copy は標準の項目をそのまま最上段**に置き
  (自前で複製しない — 選択の有無による disabled 管理も ⌘C も標準に任せる)、その隣に
  **Copy as Markdown(⇧⌘C・ショートカット併記)**を足す。Copy = 選択部分を表示どおりに。
