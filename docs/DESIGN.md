...
- **一覧(左)**: `conversations search` の結果を**全部**出す(実行中の会話も含む)。新しい順・
  ...
  disabled にし、**ボタンのすぐ左隣**に `This conversation is running` の一言を淡く添える
  (パネルの左端ではない — ボタンと一組で読む。押せないではなく
...
- **Resume ボタン**(実行できるのは past の行のみ。live では disabled — 上記): `shiibar-cc resume --cwd <cwd> --terminal <t> <session_id>` を subprocess
