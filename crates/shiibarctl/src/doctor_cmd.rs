//! `shiibarctl doctor` (DESIGN.md §4.4): checks socket connectivity / hooks
...
use crate::exitcode;
use shiibar_client::iterm::{AppleScriptRunner, ItermError, ProbeOutcome, probe};
use shiibar_proto::{InfoResponse, Request};
use std::ffi::OsStr;
use std::path::Path;
...
pub fn run_doctor(
    ...
    match shiibar_client::connection::request::<InfoResponse>(socket_path, &Request::Info) {
        Ok(info) => {
            lines.push(format!(
        ...
        Err(e) => {
            lines.push(format!(
                "[fail] daemon not reachable at {}: {e} (start it with `shiibard --foreground`)",
                socket_path.display()
            ));
    ...
    if shiibarctl_on_path(path_env).is_some() {
        lines.push("[ok]   shiibarctl is on PATH".to_string());
    } else {
        lines.push("[warn] shiibarctl is not on PATH (hooks/report.sh needs it)".to_string());
    }
...
fn shiibarctl_on_path(path_env: Option<&OsStr>) -> Option<std::path::PathBuf> {
    let path_env = path_env?;
    std::env::split_paths(path_env)
        .map(|dir| dir.join("shiibarctl"))
        .find(|p| p.is_file())
}
...
mod tests {
    ...
    #[test]
    fn shiibarctl_on_path_finds_an_executable_named_shiibarctl() {
        let dir = tempfile::tempdir().unwrap();
        std::fs::write(dir.path().join("shiibarctl"), "#!/bin/sh\n").unwrap();
        let path_env = std::env::join_paths([dir.path()]).unwrap();
        assert!(shiibarctl_on_path(Some(&path_env)).is_some());

        let other_dir = tempfile::tempdir().unwrap();
        let other_path_env = std::env::join_paths([other_dir.path()]).unwrap();
        assert!(shiibarctl_on_path(Some(&other_path_env)).is_none());
    }
