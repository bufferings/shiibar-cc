//! `shiibarctl focus <selector>` / `focus -` / `focused` (DESIGN.md §4.4).
//!
...
use crate::exitcode;
use shiibar_client::iterm::{AppleScriptRunner, ItermError, focus, focused};
...
pub fn run_focus(
    ...
    let dest_target = match selector {
        Selector::Target(t) => t,
        Selector::Cwd(_) => {
            let agents = match shiibar_client::connection::request::<ListResponse>(
                socket_path,
                &Request::List,
            ) {
                Ok(resp) => resp.agents,
                Err(e) => return err(exitcode::ERROR, format!("shiibarctl focus: {e}")),
            };
            ...
            match resolve_selector(&selector, &agents) {
                ...
                Err(SelectError::NoMatch) => {
                    return err(
                        exitcode::NOT_FOUND,
                        "shiibarctl focus: no agent matches the given selector",
                    );
                }
                Err(SelectError::Ambiguous(n)) => {
                    return err(
                        exitcode::ERROR,
                        format!(
                            "shiibarctl focus: selector matches {n} agents; use an exact target"
                        ),
                    );
                }
            }
...
/// shiibard-tracked agent, so this bypasses selector/list resolution
...
pub fn run_focus_back(
    ...
    let Some(dest_target) = read_last_focus(last_focus_path) else {
        return err(
            exitcode::NOT_FOUND,
            "shiibarctl focus -: no previous focus recorded",
        );
    };
    jump_to(socket_path, last_focus_path, &dest_target, runner)
}
...
pub fn run_focused(runner: &dyn AppleScriptRunner) -> FocusedReport {
    match focused(runner) {
        ...
        Err(ItermError::PermissionDenied) => FocusedReport {
            ...
            target: None,
            message: Some(
                "shiibarctl focused: osascript automation permission for iTerm2 is denied"
                    .to_string(),
            ),
        },
        Err(e) => FocusedReport {
            ...
            target: None,
            message: Some(format!("shiibarctl focused: {e}")),
        },
    }
}
...
fn jump_to(
    ...
    let before = match focused(runner) {
        ...
        Err(ItermError::PermissionDenied) => {
            return err(
                exitcode::TCC_DENIED,
                "shiibarctl focus: osascript automation permission for iTerm2 is denied",
            );
        }
        Err(e) => return err(exitcode::ERROR, format!("shiibarctl focus: {e}")),
    };
    ...
    if let Err(e) = focus(dest_target, runner) {
        return match e {
            ItermError::NoMatch => err(
                exitcode::NOT_FOUND,
                "shiibarctl focus: no matching iTerm2 session (tab may be closed)",
            ),
            ItermError::PermissionDenied => err(
                exitcode::TCC_DENIED,
                "shiibarctl focus: osascript automation permission for iTerm2 is denied",
            ),
            ItermError::Other(msg) => err(exitcode::ERROR, format!("shiibarctl focus: {msg}")),
        };
    }
    ...
    {
        eprintln!("shiibarctl focus: warning: failed to save last_focus: {e}");
    }
    ...
    if let Err(e) = shiibar_client::connection::request::<AckResponse>(
        socket_path,
        &Request::Seen {
            target: dest_target.to_string(),
        },
    ) {
        eprintln!("shiibarctl focus: warning: failed to notify daemon (seen): {e}");
    }
...
mod tests {
    use super::*;
    use shiibar_client::iterm::AppleScriptOutput;
    use std::sync::Mutex;
