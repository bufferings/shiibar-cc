...
use shiibar_proto::{Agent, ListResponse, Request, Status};
use std::path::Path;
...
pub fn run_list(socket_path: &Path, json: bool) -> ListReport {
    if json {
        match shiibar_client::connection::request_raw(socket_path, &Request::List) {
            Ok(line) => ListReport {
            ...
            Err(e) => ListReport {
                ...
                stdout: String::new(),
                stderr: Some(format!("shiibarctl list: {e}")),
            },
        }
    } else {
...
mod tests {
    ...
    fn table_has_one_aligned_row_per_agent() {
        let agents = vec![
            agent("t1", Status::Blocked, "/home/x/a/b", 90),
        ...
        assert!(lines[1].starts_with("idle   ")); // padded to "blocked"'s width
    }
}
