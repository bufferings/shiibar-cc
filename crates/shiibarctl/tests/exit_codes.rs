...
//! compiled `shiibarctl` and (where needed) a real `shiibard` subprocess:
...
use shiibar_proto::{HookEvent, NotificationType, SessionStartSource};
use std::process::{Command, Stdio};
use support::{TestDaemon, report_payload, shiibarctl};
...
fn list_exits_1_when_daemon_absent() {
    let dir = tempdir();
    let out = shiibarctl(dir.path(), &["list"]);
...
fn watch_exits_1_when_daemon_absent() {
    let dir = tempdir();
    let out = shiibarctl(dir.path(), &["watch"]);
    assert_eq!(out.code, 1);
}
...
fn remove_exits_1_when_daemon_absent() {
    let dir = tempdir();
    let out = shiibarctl(dir.path(), &["remove", "some-target"]);
    assert_eq!(out.code, 1);
}
...
fn doctor_exits_1_when_daemon_absent() {
    let dir = tempdir();
    let out = shiibarctl(dir.path(), &["doctor"]);
    assert_eq!(out.code, 1);
...
fn doctor_exits_0_when_daemon_is_reachable() {
    ...
    let out = shiibarctl(&daemon.state_dir, &["doctor"]);
...
fn remove_exits_2_when_selector_matches_nothing() {
    let dir = tempdir();
    let daemon = TestDaemon::start(dir.path());
    let out = shiibarctl(&daemon.state_dir, &["remove", "no-such-target"]);
...
fn remove_succeeds_with_an_exact_target_selector() {
    ...
    let out = shiibarctl(&daemon.state_dir, &["remove", "t-exact"]);
    assert_eq!(out.code, 0, "stderr={}", out.stderr);
}
...
fn dot_selector_resolves_by_current_directory() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_shiibarctl"))
        ...
        .env("SHIIBAR_STATE_DIR", &daemon.state_dir)
        .stdout(Stdio::null())
        .stderr(Stdio::null())
        .status()
        .unwrap();
...
fn list_json_forwards_the_wire_response() {
    ...
    let out = shiibarctl(&daemon.state_dir, &["list", "--json"]);
...
fn list_text_form_shows_status_and_target() {
    ...
    let out = shiibarctl(&daemon.state_dir, &["list"]);
    assert_eq!(out.code, 0);
