//! `shiibar-cc resume --cwd <dir> <session-id>` (DESIGN.md §4.3/§4.4): open
//! a new iTerm2 window and resume a past conversation there. Called by the
//! Conversations window's Resume button (§4.6), and usable standalone from
//! a script. Unlike `focus`/`seen`/`remove`, this does NOT connect to the
//! daemon and does NOT resolve a selector — `cwd` and `session_id` are
//! taken as the caller's explicit, exact values (§4.4: "resume doesn't know
//! about conversations, that's the conversations module's job, §4.6").

use crate::exitcode;
use shiibar_cc_client::iterm::{AppleScriptRunner, ItermError, open_resume_window};
...
/// Parsed `--cwd <dir> <session-id>` arguments. Pure token parsing only —
/// no filesystem access here (that's `run_resume`'s job, so the "cwd must
/// be absolute and exist" check, DESIGN.md §4.4, can be tested without
/// touching a real directory for the parsing half).
...
pub fn parse_resume_args(args: &[String]) -> Result<ResumeArgs, String> {
    ...
    let Some(cwd) = cwd else {
        return Err("usage: shiibar-cc resume --cwd <dir> <session-id>".to_string());
    };
    let Some(session_id) = session_id else {
        return Err("usage: shiibar-cc resume --cwd <dir> <session-id>".to_string());
    };
    Ok(ResumeArgs { cwd, session_id })
}
...
/// recorded in, §4.4), then open a new iTerm2 window there running
/// `claude --resume <session_id>`.
pub fn run_resume(cwd: &Path, session_id: &str, runner: &dyn AppleScriptRunner) -> ResumeReport {
    if !cwd.is_absolute() {
        return err(
            exitcode::ERROR,
            format!(
    ...
    match open_resume_window(cwd_str, session_id, runner) {
        Ok(()) => ok(),
        Err(ItermError::PermissionDenied) => err(
            exitcode::TCC_DENIED,
        ...
        Err(ItermError::NoMatch) => err(
            exitcode::ERROR,
            "shiibar-cc resume: unexpected internal error (no-match result)",
        ),
        Err(ItermError::Other(msg)) => err(exitcode::ERROR, format!("shiibar-cc resume: {msg}")),
    }
}
...
mod tests {
    ...
    #[test]
    fn parses_cwd_before_session_id() {
    ...
    fn relative_cwd_is_exit_1_and_never_touches_osascript() {
        ...
        let report = run_resume(Path::new("relative/path"), "SESSION-1", &PanicRunner);
    ...
    fn nonexistent_cwd_is_exit_1_and_never_touches_osascript() {
        ...
        let report = run_resume(&missing, "SESSION-1", &PanicRunner);
        assert_eq!(report.exit_code, exitcode::ERROR);
        assert!(report.message.is_some());
    }
    ...
    fn successful_resume_is_exit_0() {
        ...
        let report = run_resume(dir.path(), "SESSION-1", &runner);
