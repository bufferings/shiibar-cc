//! Shared harness for shiibard integration tests: spin up a real in-process
//! server bound to a temp state dir, and talk to it over real
//! `tokio::net::UnixStream` connections — exactly what `shiibarctl` and a
//! subscriber do, just without spawning the compiled binaries.
#![allow(dead_code)]

use shiibard::clock::Clock;
use shiibard::core::Core;
use shiibard::logging::{Level, Logger};
use shiibard::paths::StateDir;
use shiibard::server;
use shiibar_proto::{codec, Request, SubscribeEvent};
...
impl TestDaemon {
    /// Start a real server bound inside `dir` (a temp `SHIIBAR_STATE_DIR`).
    ...
    pub async fn report(&self, payload: shiibar_proto::ReportPayload) {
