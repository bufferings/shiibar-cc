...
use shiibar_proto::SessionRecord;
...
impl SessionStore {
    ...
    pub fn append(&mut self, record: SessionRecord) -> anyhow::Result<()> {
        let line = shiibar_proto::codec::encode_line(&record)?;
    ...
    fn rewrite_compacted(&mut self) -> anyhow::Result<()> {
        ...
        for rec in entries {
            out.push_str(&shiibar_proto::codec::encode_line(rec)?);
        }
...
/// while keeping `shiibar_proto::SessionRecord` itself simple.
