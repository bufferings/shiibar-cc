//! Message types and NDJSON codec shared by shiibard and shiibarctl.
//!
...
/// doesn't know about must not fail the line). shiibard itself never
/// constructs `Unknown`.
...
/// Hook event kind, as normalized by `shiibarctl report` (§4.1) and sent to
/// shiibard over the wire (§4.2). This is a closed set for M1: the seven
...
/// A report from `shiibarctl report`, normalized (§4.1) and already
