//! Minimal test-only harness: a real in-process shiibard bound to a temp
//! `SHIIBAR_STATE_DIR`, run on its own OS thread with its own tokio
...
use shiibard::logging::{Level, Logger};
...
impl TestDaemon {
    ...
    pub fn start() -> Self {
        ...
        let thread = std::thread::spawn(move || {
            ...
            rt.block_on(async move {
                ...
                let (events_tx, _rx) =
                    tokio::sync::broadcast::channel(shiibard::core::BROADCAST_CAPACITY);
    ...
    pub fn report(&self, payload: shiibar_proto::ReportPayload) {
        ...
        let line =
            codec::encode_line(&shiibar_proto::Request::Report(payload)).expect("encode report");
    ...
    pub fn remove(&self, target: &str) {
        ...
        let req = shiibar_proto::Request::Remove {
    ...
    pub fn shutdown(mut self) {
        if let Ok(mut stream) = UnixStream::connect(&self.sock_path) {
            let line =
                codec::encode_line(&shiibar_proto::Request::Shutdown).expect("encode shutdown");
